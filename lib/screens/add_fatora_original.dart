...
class _AddFatoraWithItemsState extends State<AddFatoraWithItems> {
  ...
  Future<void> _createText(Map<String,String> map ,String name) async{
    ...
    final List<int> bytes = workbook.saveAsStream();
    FileStorage.writeCounter(map.toString(), "Fatora.txt", );
                                                ...
                                                onSaved: (value){
                                                  if(value!.isNotEmpty &&value!.length>0)
                                                  {
                                                    if(itemsList.isNotEmpty){
                                                      itemsList.add('Item${itemsList.length} $value');
                                                    }
                                                    if(itemsList.isEmpty){
                                                      itemsList.insert(0 , 'Item0 $value');
                                                    }
                                                  }
                    ...
                    if(index == itemsList.length-1){
                      return Container(
                        ...
                        child: Directionality(
                          textDirection: TextDirection.rtl,
                          child: TextFormField(
                            initialValue:itemsList[index].contains('Item') ? itemsList[index].substring(6):
                            itemsList[index] ,
                            decoration: InputDecoration(
                              ...
                              filled: true,
                              alignLabelWithHint: false,
                              label: Text((itemsList[index].startsWith('Item'))?'اسم الصنف':'سيريال رقم ${index+1}',
                                style: TextStyle(fontWeight: FontWeight.bold ,
                                    color:itemsList[index].startsWith('Item') ? Colors.black:
                                    Colors.black), textAlign: TextAlign.right,),
                              fillColor: itemsList[index].startsWith('Item') ?Colors.green :
                            ...
                            onChanged: (value){
                              if(itemsList[index].contains("Item")){
                                itemsList[index]="Item ${value}";
                              }
                              if( !itemsList[index].contains("Item")){
